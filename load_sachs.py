...
import helper_em_tf, helper_em, helper_tf_model
import helper_data
...
bool_ratio = 0
n_steps = 40
seed = 812#840#932#629

reg_lmbda = 1e-25
attack_node = 'pip2'

bool_hat_true = True
#bool_force_zero = True
...
if mode_adv == 0:
    S_a, mu_a, idx_adv_train = helper_data.get_adv_sachs(mu, S)
    W_a  = helper_data.get_adv_sachs(mu, S, W = W)[0]
...
debug = 0
if debug:
    S_0 = np.eye(X.shape[1]).astype(np.float32)
    mu_0 = np.zeros(X.shape[1]).astype(np.float32)
    x = tf.constant(X, tf.float32)
    bool_history = 1
    temp_load = helper_em_tf.get_em_enum(x, p_r_x, 
                                                 idx_adv_train,
                                                 mu_0 = mu_0, 
                                                 S_0 = S_0,
                                                 n_steps = n_steps,
    ...
    if bool_history:
        ...
        fig.savefig(os.path.join(currPath,'history.png'), 
